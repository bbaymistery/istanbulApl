...
const DesktopMenu = ({ language, journeyType, airportTranslations }) => {
    ...
    return (
        <div className={styles.header_menu_content}>
            <ul>
                {navigator.map(({ path, innerText, list, type, title, strInnerText }, index) => {
                    ...
                    return (
                        <li key={innerText} className={`${styles.li_item} ${type === 'list' ? styles.has_children : ''}`}  >
                            {index === 0 ? (
                                ...
                                <a href={fullPath} title={title} className={`${path.length ? styles.nocursor : ''} ${isActive ? styles.active : ''}`} target={index === 4 ? '_blank' : ''}    >
                                    <span>{translatedText}</span>
